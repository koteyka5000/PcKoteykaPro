...
def read(cell=-1):  # Прочитать файл для записи. Вернёт все значения при вызове без аргументов, или нужную дробь
...
class Fraction:
    ...
    def __floordiv__(self, other):  # Деление, но вернёт число (float) //
        q = self.numer * other.denom
        w = self.denom * other.numer
        return q / w
